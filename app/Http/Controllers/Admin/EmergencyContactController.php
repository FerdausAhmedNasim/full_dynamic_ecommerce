...
class EmergencyContactController extends Controller
{
    ...
    public function showCreateForm(User $user)
    {
        if ($user->user_type == Enum::USER_TYPE_ADMIN || $user->user_type == Enum::USER_TYPE_EMPLOYEE) {
            $route = route('admin.user.employee.show', $user?->employee->id) . '#emergency-contact';
        } elseif ($user->user_type == Enum::USER_TYPE_CUSTOMER || $user->user_type == Enum::USER_TYPE_DAY_MEMBER) {
            $route = route('admin.member.show.contact', $user?->member->id);
        } elseif ($user->user_type == Enum::USER_TYPE_SKIPPER) {
            $route = route('admin.skipper.show.contact', $user->id);
        }
    ...
    public function create(User $user, CreateRequest $request)
    {
        ...
        if ($result) {
            if ($user->user_type == Enum::USER_TYPE_ADMIN || $user->user_type == Enum::USER_TYPE_EMPLOYEE) {
                return redirect(route('admin.user.employee.show', $user?->employee->id) . '#emergency-contact')->with('success', $this->emergency_service->message);
            } elseif ($user->user_type == Enum::USER_TYPE_CUSTOMER || $user->user_type == Enum::USER_TYPE_DAY_MEMBER) {
                return redirect(route('admin.member.show.contact', $user?->member->id))->with('success', $this->emergency_service->message);
            } elseif ($user->user_type == Enum::USER_TYPE_SKIPPER) {
                return redirect(route('admin.skipper.show.contact', $user->id))->with('success', $this->emergency_service->message);
            }
    ...
    public function update(EmergencyContact $emergency, UpdateRequest $request)
    {
        ...
        if ($result) {
            if ($emergency?->user?->user_type == Enum::USER_TYPE_ADMIN || $emergency?->user?->user_type == Enum::USER_TYPE_EMPLOYEE) {
                return redirect(route('admin.user.employee.show', $emergency?->user?->employee->id) . '#emergency-contact')->with('success', $this->emergency_service->message);
            } elseif ($emergency?->user?->user_type == Enum::USER_TYPE_CUSTOMER || $emergency?->user?->user_type == Enum::USER_TYPE_DAY_MEMBER) {
                return redirect(route('admin.member.show.contact', $emergency?->user?->member->id))->with('success', $this->emergency_service->message);
            } elseif ($emergency?->user->user_type == Enum::USER_TYPE_SKIPPER) {
                return redirect(route('admin.skipper.show.contact', $emergency?->user->id))->with('success', $this->emergency_service->message);
            }
