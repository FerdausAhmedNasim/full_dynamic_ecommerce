...
class ProductController extends Controller
{
    ...
    public function variants(Request $request)
    {
        if ($request->has_variant == 1) {
            ...
            if (!empty($variants_data[0])) {
                return view('admin.pages.product.partials.variants', [
                    'variants'      => $variants,
    ...
    public function variantsEdit(Product $product, Request $request)
    {
        if ($request->has_variant == 1) {
            ...
            if (!empty($variants_data[0])) {
                return view('admin.pages.product.partials.variants_edit', [
                    'product'       => $product,
                    'product_price' => $product_price,
                    'variants'      => $variants,
                    'variants_data' => $variants_data,
    ...
    public function getAttributeValues(Request $request)
    {
        ...
        if (!empty($attributes_sets)) {
            ...
            return view('admin.pages.product.partials.values', [
    ...
    public function changeTodayDeal(Request $request, Product $product)
    {
        $result = $this->product_service->changeTodayDeal($product);
    ...
    public function changeRefundStatus(Request $request, Product $product)
    {
        $result = $this->product_service->changeRefundStatus($product);
    ...
    public function showAlertProducts(Request $request)
    {
        ...
        return view('admin.pages.product.alert');
    }
    ...
    {
        $data = ProductReview::with('product.productLanguages', 'customer')->get();

        if ($request->ajax()) {
            return $this->product_service->productReviewDataTable($data);
        }

        return view('admin.pages.review.show_allReview');
    }
    ...
    public function ezzicoDiscount(Request $request)
    {
        $filter_discount = EzzicoSale::where("product_id", $request->product_id)->first();
        $ezzico_sale = $request->all();
        
        if ($filter_discount) {
            $result = $filter_discount->update($ezzico_sale);
            ...
            $result = EzzicoSale::create($ezzico_sale);
        ...
        return back()->withInput($request->all())->with('error', 'Something Wrong');
    }
}
