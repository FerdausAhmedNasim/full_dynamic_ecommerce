...
class QuestionAnswerService extends BaseService
{
    ...
    {
        $actionHtml = '';
        $route = route('admin.product.question.delete', [$row->product_id, $row->id]);

        if ($row->id) {
            $answer = ProductQuestion::where('parent_id', $row->id)->first();

            if (Helper::hasAuthRolePermission('product_question_answer') && !$answer) {
                $actionHtml .= '<a class="dropdown-item text-secondary" href="javascript:void(0)" onclick="clickAnswerModal(' . $row->id . ', ' . $row->product->id . ')" ><i class="far fa-edit"></i> Answer </a>';
            }

            if (Helper::hasAuthRolePermission('product_question_delete')) {
    ...
    public function productQuestionDataTable()
    {
        $data = ProductQuestion::with('customer', 'product')->whereNull('parent_id')->get();

        return Datatables::of($data)
                ->addIndexColumn()
                ->editColumn('product_id', function ($row) {
                    $name = Str::limit($row?->product->getTranslation('title'), 30);
                    $route = url(config('app.url') . '/products/show/' . $row->product->slug);

                    return '<a href="' . $route . '" class="text-primary pl-2" target="_blank">' . $name . '</a>';
                })
                ->editColumn('customer_id', function ($row) {
                    return $row?->customer?->full_name ?? 'Anonymous';
                })
                ...
                ->rawColumns(['product_id', 'answer', 'active', 'action'])
                ->make(true);
    }
    ...
    private function getSwitch($row)
    {
        ...
        $permission = Helper::hasAuthRolePermission('product_question_change_status');

        if ($permission) {
            $disabled = '';
            $route = "'" . route('admin.product.question.change_status', [$row->product_id, $row->id]) . "'";
        }
