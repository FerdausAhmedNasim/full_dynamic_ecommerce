...
class PickupHubService extends BaseService
{
    ...
    {
        $actionHtml = '';
        $route = route('admin.config.general_settings.pickup_hub.delete', $row->id);

        if ($row->id) {
            if (Helper::hasAuthRolePermission('pickup_hub_update')) {
                $actionHtml .= '<a class="dropdown-item text-primary" href="' . route('admin.config.general_settings.pickup_hub.edit', $row->id) . '" ><i class="far fa-edit"></i> Edit</a>';
            }

            if (Helper::hasAuthRolePermission('pickup_hub_delete')) {
    ...
    {
        $data = Address::where('user_id', sellerId())->get();
