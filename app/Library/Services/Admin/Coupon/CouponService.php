...
namespace App\Library\Services\Admin\Coupon;
...
class CouponService extends BaseService
{
    ...
    {
        $actionHtml = '';
        $route = route('admin.coupon.delete', $row->id);

        if ($row->id) {
            if (Helper::hasAuthRolePermission('coupon_update')) {
                $actionHtml .= '<a class="dropdown-item" href="' . route('admin.coupon.edit', $row->id) . '" ><i class="far fa-edit"></i> Edit</a>';
            }

            if (Helper::hasAuthRolePermission('coupon_delete')) {
    ...
    public function dataTable()
    {
        ...
        return Datatables::of($data)
                ...
                ->editColumn('maximum_discount', function ($row) {
                    return $row->maximum_discount ? getFormattedAmount($row->maximum_discount) : "N/A";
                })
