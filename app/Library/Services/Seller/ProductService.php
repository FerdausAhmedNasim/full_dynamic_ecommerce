...
class ProductService extends BaseService
{
    private function actionHtml($row)
    {
        ...
        if ($row->id) {
            if (Helper::hasAuthRolePermission('seller_product_show')) {
                $actionHtml .= '<a class="dropdown-item" href="' . $route . '" target="_blank" ><i class="far fa-eye"></i> Show </a>';
            }

            if (Helper::hasAuthRolePermission('seller_product_update')) {
                $actionHtml .= '<a class="dropdown-item" href="' . route('seller.product.edit', $row->id) . '" ><i class="far fa-edit"></i> Edit </a>';
            }

            if (Helper::hasAuthRolePermission('seller_product_update')) {
                $actionHtml .= '<a class="dropdown-item" href="' . route('seller.product.reviews', $row->id) . '" ><i class="far fa-star"></i> Review </a>';
            }

            if (Helper::hasAuthRolePermission('seller_product_update')) {
                $actionHtml .= '<a class="dropdown-item" href="' . route('seller.product.question.index', $row->id) . '" ><i class="fa-regular fa-comment-dots"></i> Answer </a>';
            }

            // if (Helper::hasAuthRolePermission('seller_product_create_clone')) {
            //     $actionHtml .= '<a class="dropdown-item" href="' . route('seller.product.clone', $row->id) . '" ><i class="fa-regular fa-clone"></i> Clone </a>';
            // }

            if (Helper::hasAuthRolePermission('seller_product_delete')) {
                $actionHtml .= '<a style="cursor: pointer" class="dropdown-item text-danger" onclick="confirmFormModal(\'' . route('seller.product.delete', $row->id) . '\', \'Confirmation\', \'Are you sure to delete operation?\')" ><i class="fas fa-trash-alt"></i> Delete</a>';
            }
        }
    ...
    {
        $query = Product::with('category', 'brand', 'operator', 'productLanguages', 'productStock', 'productStocks')->where('seller_id', authSellerId());
    ...
    public function dataTable($filter_params)
    {
        ...
        return Datatables::of($data)
                ...
                //     return $this->getDisabledSwitch($row, 'featured');
                // })
                ->editColumn('refundable', function ($row) {
                    $route = "'" . route('seller.product.refundable', $row->id) . "'";
                ...
                ->editColumn('showHomePage', function ($row) {
                    $route = "'" . route('seller.product.showHomePage', $row->id) . "'";
                    $is_check = $row->show_home_page ? "checked" : "";

                    if ($row->status == Enum::PRODUCT_STATUS_PUBLISHED && $row->deleted_at == null) {
                        return $this->getSwitch($row, $route, $is_check, 'showHomePage');
                    }

                    return $this->getDisabledSwitch($row, 'showHomePage');
                })
                ->editColumn('approved', function ($row) {
                    return getProductApprovedStatus($row?->approved);
                })
    ...
    {
        $data = Product::with('productStock')->get()->filter(function($product) {
    ...
    public function getDetails($row)
    {
        ...
        $detailsHtml = '';
        $detailsHtml .= '<span>Price: ' . getFormattedAmount($row->unit_price) . $unit . '</span> <br />';
        $detailsHtml .= '<span>Total Sale: ' . $row->total_sale . '</span> <br />';
        $detailsHtml .= '<span>Rating: ' . $row->rating . '</span> <br />';
        $detailsHtml .= '<span>Current Stock: ' . $row->current_stock . '</span>';
    ...
    public function getCurrentStock($row)
    {
        ...
        foreach ($row->productStocks as $stock) {
            ...
            $currentStock .= '<span>' . $stock->name . $separator . $stock->current_stock . '</span> <br />';
        }
    ...
    {
        if ($row->approved && $row->status != Enum::PRODUCT_STATUS_TRASH && $row->deleted_at == null) {
            $route = "'" . route('seller.product.change_status', $row->id) . "'";

            $is_check = $row->status == Enum::PRODUCT_STATUS_PUBLISHED ? "checked" : "";

            return $this->getSwitch($row, $route, $is_check, 'status');
        } elseif (!$row->approved) {
            return '<span class="badge bg-warning" style="color: #fff;">Pending</span>';
        }

        return $this->getDisabledSwitch($row, 'status');
    }
    ...
    private function getDisabledSwitch($row, $type = '')
    {
        $is_check = "";

        if ($row->status == Enum::PRODUCT_STATUS_PUBLISHED && $type == 'status') {
            $is_check = 'checked';
        } elseif ($row->featured && $type == 'featured') {
            $is_check = 'checked';
        } elseif ($row->refundable && $type == 'refundable') {
            $is_check = 'checked';
        } elseif ($row->show_home_page && $type == 'showHomePage') {
            $is_check = 'checked';
        }

        return '<label class="cursor-not-allowed custom-switch" >
                    <input type="checkbox" name="refundable" class="custom-switch-input" disabled ' . $is_check . '>
                    <span class="custom-switch-indicator"></span>
                </label>';
    }
    ...
    public function store(array $data): bool
    {
        ...
        try {
            ...
            // Product Table
            $productTblData = [];
            $productTblData['seller_id'] = authSellerId();
            ...
            $productTblData['show_home_page'] = isset($data['showHomePage']) ? $data['showHomePage'] : false;
            ...
            if (isset($data['product_thumbnail']) && $data['product_thumbnail'] != '') {
                attachmentStore($data['product_thumbnail'], $product, Enum::PRODUCT_THUMBNAIL_IMAGE_DIR, Enum::ATTACHMENT_TYPE_THUMBNAIL, 750 , 750);
            }
    ...
    {
        try {
            if ($product->status == Enum::PRODUCT_STATUS_UNPUBLISHED) {
                $status = Enum::PRODUCT_STATUS_PUBLISHED;
            } elseif ($product->status == Enum::PRODUCT_STATUS_PUBLISHED) {
                $status = Enum::PRODUCT_STATUS_UNPUBLISHED;
            }
    ...
    public function isShowHomePage(Product $product): bool
    {
        try {
            $this->data = $product->update(['show_home_page' => !$product->show_home_page]);
    ...
    public function storeCloneProduct(Product $product, array $data): bool
    {
        DB::BeginTransaction();
        ...
        try {
            ...
            $cloneProduct = Product::create($productTblData);

            // Product language Table
            $productLanTblData['product_id'] = $cloneProduct->id;
            $productLanTblData['local'] = 'en';
            $productLanTblData['title'] = $data['title'];
            $productLanTblData['short_description'] = $data['short_description'];
            $productLanTblData['description'] = $data['description'];
            ...
            // Product Details Table
            $productDetailsTblData['product_id'] = $cloneProduct->id;
            // $productDetailsTblData['shipping_type'] = $data['shipping_type'];
            // $productDetailsTblData['shipping_fee'] = $data['shipping_fee'] ?? 0;
            // $productDetailsTblData['shipping_fee_depend_on_quantity'] = $data['shipping_fee_depend_on_quantity'] ?? false;
            // $productDetailsTblData['shipping_fee_depend_on_weight'] = $data['shipping_fee_depend_on_weight'] ?? false;
            ...
            if (isset($data['has_variant']) && count($data['variant_name']) > 0) {
                ...
                foreach ($requestData['stock_id'] as $key => $stockID) {
                    if ($data['variant_name'][$key]) {
                        $product_stock['product_id'] = $cloneProduct->id;
                        $product_stock['variant_ids'] = $data['variant_ids'][$key];
                        $product_stock['name'] = $data['variant_name'][$key];
                        $product_stock['sku'] = $data['variant_sku'][$key];
                        $product_stock['current_stock'] = $data['variant_stock'][$key];
                        ...
                        $parentStock = ProductStock::with('attachment')->where('id', $stockID)->where('product_id', $product->id)->first();

                        if (isset($parentStock->attachment)) {
                            $attach = $parentStock->attachment;

                            attachmentCloneStore($attach, $productStock);
                        }
                    ...
                    $cloneProduct->selected_variants = $selected_variants;
                    $cloneProduct->selected_variants_ids = $selected_variants_ids;
                }

                $cloneProduct->current_stock = $total_stock;
                $cloneProduct->save();
            } else {
                $productStockTblData['product_id'] = $cloneProduct->id;
                $productStockTblData['sku'] = $data['sku'];
                $productStockTblData['current_stock'] = $data['current_stock'];
                $productStockTblData['unit_price'] = $data['unit_price'];

                ProductStock::create($productStockTblData);
            }

            if (isset($data['colors']) && $data['colors'] != '') {
                $cloneProduct->colors()->attach($data['colors']);
            }

            // Product Thumbnail
            if (isset($data['product_thumbnail']) && $data['product_thumbnail'] != '') {
                attachmentStore($data['product_thumbnail'], $cloneProduct, Enum::PRODUCT_THUMBNAIL_IMAGE_DIR, Enum::ATTACHMENT_TYPE_THUMBNAIL);
            } else {
                if ($product->getProductThumbnail()) {
                    $thumbnail = $product->getProductThumbnail();
                    attachmentCloneStore($thumbnail, $cloneProduct);
                }
            }
            ...
            if (isset($requestData['old_images'])) {
                $oldImages = $requestData['old_images'];
                $attachments = Attachment::where('attachable_type', Product::class)->where('for', Enum::ATTACHMENT_TYPE_GALLERY)->get();

                foreach ($attachments as $attachment) {
                    if (in_array($attachment->id, $oldImages)) {
                        attachmentCloneStore($attachment, $cloneProduct);
                    }
                }
            }
            ...
            if (isset($data['images']) && $data['images'] != '') {
                foreach ($data['images'] as $image) {
                    attachmentStore($image, $cloneProduct, Enum::PRODUCT_GALLERY_IMAGE_DIR, Enum::ATTACHMENT_TYPE_GALLERY);
                }
            }

            // Descriptions Image
            if ($product->getProductDescriptionImage()) {
                $descriptionImg = $product->getProductDescriptionImage();
                attachmentCloneStore($descriptionImg, $cloneProduct);
            }

            if (isset($data['description_image']) && $data['description_image'] != '') {
                attachmentStore($data['description_image'], $cloneProduct, Enum::PRODUCT_DESCRIPTION_IMAGE_DIR, Enum::ATTACHMENT_TYPE_DESCRIPTION);
            }

            // Meta Image
            if ($product->getProductMetaImageAttribute()) {
                $metaImg = $product->getProductMetaImageAttribute();
                attachmentCloneStore($metaImg, $cloneProduct);
            }

            if (isset($data['meta_image']) && $data['meta_image'] != '') {
                attachmentStore($data['meta_image'], $cloneProduct, Enum::PRODUCT_META_IMAGE_DIR, Enum::ATTACHMENT_TYPE_META);
            }
    ...
    private function getStatusSwitch($row)
    {
        ...
        $route1 = "'" . route('seller.product.review_change_status', $row->id) . "'";
