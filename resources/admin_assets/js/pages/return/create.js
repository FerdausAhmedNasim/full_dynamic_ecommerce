$(document).ready(function () {
    ...
    function updateSubtotal(seller_order_id) {
        var amount = 0;
        $(".order-table tr .sub_total").each(function () {
            amount = parseFloat(amount) + parseFloat(this.value);
        });
        $("#total-amount").html(formatPrice(amount));
        $(".total_amount").val(amount);
    }

    function updateQuantity(seller_order_id) {
        var qty = 0;
        $(".order-table tr .return-qty").each(function () {
...
// $('.submit-btn').on('click', function (e) {
    window.submitForm = function (e, id = '') { 
        var is_valid = false;
        $(".order-table tr .is_return").each(function () {
            if($(this).is(":checked")){
                is_valid = true;
            }
        });
