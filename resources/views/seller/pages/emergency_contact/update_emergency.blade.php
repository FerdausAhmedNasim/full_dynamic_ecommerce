...
<div class="content-wrapper">
    ...
    <div class="content-header d-flex justify-content-start">
        {!! \App\Library\Html::linkBack( route('seller.profile.index') . '#emergency-contact' ) !!}
        <div class="d-block">
    ...
    <div class="card shadow-sm">
        <div class="card-body py-sm-4">
            <form method="post"
                action="{{ route('seller.emergency.update', $emergency_contact->id) }}"
                enctype="multipart/form-data">
                @csrf

                <div class="row">
                    <div class="col-md-12">
                        <h3 class="col-md-12 text-center text-primary">
                            {{ $emergency_contact?->user?->full_name }} </h3>
                        <hr>
                    </div>
                                        ...
                                        <div class="input-group-prepend">
                                            <select name="country_code" class="input-group-text text-secondary"
                                                ...
                                                @foreach($countries as $key => $country)
                                                    <option
                                                        value="{{ old('country_code', $country['code']) }}"
                                                        {{ $country['code'] == (explode('-', $emergency_contact->mobile_number))[0] ? "selected" : "" }}>
                                                        {{ $key }}
                                                        {{ $country['code'] }}</option>
                                                @endforeach
                                            </select>
                            ...
                            <div class="form-group row @error('relationship') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="relationship"
                                        ...
                                        placeholder="{{ __('Ex: Brother, Sister, Father') }}"
                                        required>
                                    @error('relationship')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
