...
@section('title', __('Update Ad Request'))
...
<div class="content-wrapper">
    ...
    <div class="content-header d-flex justify-content-start">
        {!! \App\Library\Html::linkBack(route('seller.ad.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Ad Request')) }}</h4>
        </div>
    </div>

    <div class="card shadow-sm">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('seller.ad.update', $ad->id) }}"
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="p-sm-3">
                                    ...
                                    <select id="advertise_location_id" class="form-control select2" name="advertise_location_id" required readonly>
                                        ...
                                        @foreach($locations as $location)
                                            <option class="text-capitalize" value="{{ $location->id }}" data-name="{{ $location->location }}" {{ (old("advertise_location_id", $ad->advertise_location_id) == $location->id) ? "selected" : "" }}>
                                                {{ $location->location_name }} - {{ settings('currency_symbol') }}{{ $location->amount }} ({{ $location->location === \App\Library\Enum::AD_LOCATION_FLASH_SALE ? 'Every product per day' : 'Per day'}})</option>
                                        @endforeach
                                    </select>
                            ...
                            @if ($ad->adLocation->location != \App\Library\Enum::AD_LOCATION_FLASH_SALE)
                                            ...
                                            <div class="file-upload-section">
                                                ...
                                                <div class="input-group col-xs-12">
                                                    <input type="text"
                                                        ...
                                                        disabled="" readonly placeholder="Size: 800x350 and max 1024kB">
                                                ...
                                                <div class="display-input-image @if($ad->attachment == null) d-none @endif">
                                                    <img src="{{ $ad->image }}" alt="Preview Image" />
                                                    <button type="button"
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control productSelect2" name="product_ids[]" multiple>
                                            ...
                                            @foreach($products as $product)
                                                <option class="text-capitalize" value="{{ $product->thumbnail }}" @foreach ( json_decode($ad->product_ids) as $item)
                                                    {{ $item == $product->id ? "selected" : ""}}
                                                    @endforeach>
                                                    {{ $product->getTranslation('title') }}</option>
                                            @endforeach
                                        </select>
                                        @error('product_ids')
                                            <p class="error-message">{{ $message }}</p>
