...
@section('title', __('New Health'))

@section('content')
<div class="content-wrapper">

    <div class="content-header d-flex justify-content-start">
    {!! \App\Library\Html::linkBack( route('seller.profile.index') . '#health') !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('New Health')) }}</h4>
        </div>
    </div>
    <div class="card shadow-sm col-xxl-6 col-xl-6 col-lg-12 col-md-12">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('seller.health.create') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="p-sm-3">
                            ...
                            {{-- <div class="form-group row @error('vaccination_status') error @enderror">
                                <label class="col-sm-3 col-form-label required">{{ __('Vaccination Status') }}</label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="vaccination_status" value="{{ old('vaccination_status') }}"
                                        placeholder="{{ __('Vaccination Status') }}" required>
                                    ...
                                    <div class="d-inline-flex justify-content-start">
                                        <div class="form-check form-check-success mr-5">
                                            <label class="form-check-label">
                                                <input type="radio" class="form-check-input" name="vaccination_status" value="Yes" required {{ old('vaccination_status') ? 'checked' : '' }}>
                                                Yes
                                                <i class="input-helper"></i></label>
                                        </div>
                                        <div class="form-check form-check-danger">
                                            <label class="form-check-label">
                                                <input type="radio" class="form-check-input" name="vaccination_status" value="No" required {{ old('vaccination_status') ? 'checked' : '' }}>
                                                No
                                                <i class="input-helper"></i></label>
                                        </div>
                                    </div>
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="blood_group" required>
                                        ...
                                        @foreach(\App\Library\Enum::getBloodGroup() as $index => $value)
                                            <option class="text-capitalize" value="{{ $index }}" {{(old("blood_group") == $index) ? "selected" : ""}}>
                                                {{ $value }}</option>
                                        @endforeach
                                    </select>
                                    @error('blood_group')
                            ...
                            <div class="form-group row @error('medical_practice') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="medical_practice" value="{{ old('blood_group') }}"
                                ...
                                <div class="col-sm-9">
                                    <div class="d-inline-flex justify-content-start">
                                        <div class="form-check form-check-success">
                                            <label class="form-check-label">
                                            <input type="checkbox" class="form-check-input" name="disablity_status" value="{{ old('disablity_status' == 1 ? 'checked' : '', 1) }}">
                                            <i class="input-helper"></i></label>
                                        </div>
