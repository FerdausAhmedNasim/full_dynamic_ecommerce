        ...
        <div class="page-header text-primary-m1-d2">
            <h1 class="page-title text-secondary-d1">
                ...
                <small class="page-info">
                    ...
                    {{ $seller_order->order->invoice_id }}
                </small>
            </h1>
            <div class="page-tools">
                <div class="action-buttons">
                    <a class="btn bg-white btn-light mx-1px text-95"
                        href="{{ route('seller.order.invoice.download',$seller_order->id)}}" data-title="Print">
                        ...
                        <div class="col-12">
                            <div class="row d-flex justify-content-around">
                                <div class="text-center text-150 col-sm-6 d-sm-flex justify-content-center">
                                    <a class="brand-logo-mini" href="{{ route('seller.home.dashboard') }}"><img
                                            class="img-fluid"
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="text-grey-m2">
                                <div class="my-1"><i class="fa-solid fa-user fa-flip-horizontal text-secondary"></i> <b
                                        class="text-600 text-primary-m1">{{ $seller_order?->order->customer->full_name }}</b></div>
                                <div class="my-1"><i
                                        class="fa-solid fa-map-location-dot fa-flip-horizontal text-secondary"></i> <b
                                        class="text-600">{{ $seller_order?->order?->address?->full_address??' N/A' }}</b></div>

                                <div class="my-1"><i class="fa fa-phone text-secondary"></i> <b
                                        class="text-600">{{ $seller_order?->order->customer->phone ?? ' N/A' }}</b></div>
                                <div class="my-1"><i class="fa fa-envelope fa-flip-horizontal text-secondary"></i> <b
                                        class="text-600">{{ $seller_order?->order->customer->email ?? ' N/A'  }}</b></div>
                            </div>
                        </div>
                            ...
                            <div class="text-grey-m2">
                                ...
                                <div class="text-grey-m2">
                                    ...
                                    <div class="my-1"><i
                                        ...
                                        <b class="text-600">{{ getCompanyAddress() == '' ? 'N/A' : getCompanyAddress() }}</b></div>

                                    <div class=""><i class="fa fa-phone text-secondary"></i>
                                        <b class="text-600">{{ settings('phone') == '' ? 'N/A' : settings('phone')}}</b></div>
                                    <div class="my-1"><i class="fa fa-envelope fa-flip-horizontal text-secondary"></i>
                                        <b class="text-600">{{ settings('email') == '' ? 'N/A' : settings('email')}}</b></div>
                                    <div class=""><i class="fa fa-globe fa-flip-horizontal text-secondary"></i>
                                        <b class="text-600"><a class="text-primary-m1" target="_blank" href="{{ settings('website') && settings('website') != '' ? settings('website') : '#' }}">{{ settings('website') && settings('website') != '' ? settings('website') : 'N/A' }}</a></b></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="text-95 col-sm-12 align-self-start d-sm-flex justify-content-center"
                            ...
                            <div class="text-grey-m2">
                                ...
                                <div class="mx-3 d-inline"><i class="fa fa-circle text-primary-m1 text-xs mr-1"></i>
                                    <span class="text-600 text-90">Invoice Number:</span> {{ $seller_order->order->invoice_id }}
                                </div>
                                <div class="mx-3 d-inline"><i class="fa fa-circle text-primary-m1 text-xs mr-1"></i>
                                ...
                                <div class="mx-3 d-inline"><i class="fa fa-circle text-primary-m1 text-xs mr-1"></i>
                                    ...
                                    {!! getOrderPaymentStatus($seller_order->payment_status) !!}
                                    {{-- <span
                                        class="badge badge-warning badge-pill px-25">{{ \App\Library\Enum::getPaymentStatusType($seller_order?->payment_status) }}
                                    </span> --}}
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-4">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                @foreach ($seller_order->sellerOrderDetails as $key => $seller_orderDetail)
                                @php
                                $qty = $qty + $seller_orderDetail->quantity;
                                $subtotal = $seller_orderDetail->quantity*$seller_orderDetail->sale_price;
                                @endphp
                                <tr>
                                    <td class="text-center">{{++$key}}</td>
                                    <td>
                                        <img width="50" src="{{ $seller_orderDetail?->product?->getThumbnailImage() }}">
                                    </td>
                                    <td class="text-left">{{ $seller_orderDetail?->product->getTranslation('short_title') }}
                                        @if ($seller_orderDetail?->load('productStock')?->productStock?->variant_ids)
                                            <br> <small>{{ getProductVariantValue($seller_orderDetail?->load('productStock')?->productStock?->variant_ids) }}</small>
                                        @endif
                                    </td>
                                    <td class="text-right">{{ $seller_orderDetail->quantity }}</td>
                                    <td class="text-right">{{ formatPrice($seller_orderDetail->sale_price) }}</td>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="text-right">{{ formatPrice($seller_order->sub_total_amount) }}</td>
                                </tr>
                                <tr>
                                    <td colspan="5" class="text-right">Discount</td>
                                    <td class="text-right"><span
                                            class="text-120 text-secondary-d1">{{ getFormattedAmount($seller_order->discount_amount) }}</span>
                                    </td>
                                </tr>

                                <tr>
                                    <th colspan="5" class="text-right text-150">Total <small>({{ numberToWord($seller_order->total_amount) }})</small></th>
                                    <td class="text-right"><span class="text-150 text-success-d3 opacity-2">
                                            {{formatPrice($seller_order->total_amount)}}</span></td>
                                </tr>
                            </tfoot>
                        </table>
                        ...
                        <div>
                            ...
                            <a class="btn mb-2 mr-2 btn-success px-4 float-right mt-3 mt-lg-0"
                                href="{{ route('seller.order.show', $seller_order->id) }}">
