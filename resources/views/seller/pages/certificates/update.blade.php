...
@section('title', __('Update Certificates'))
...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('seller.profile.index') . '#tab-certificates') !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Certificates')) }}</h4>
        </div>
    ...
    <div class="card shadow-sm">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('seller.certificate.update', $training->id) }}" enctype="multipart/form-data">
                @csrf
                            ...
                            <div class="form-group row @error('facilitator') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="facilitator"
                                        value="{{ old('facilitator', $training->facilitator) }}" placeholder="{{ __('Facilitor') }}"
                            ...
                            <div class="form-group row @error('training_place') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="training_place"
                                        value="{{ old('training_place', $training->training_place) }}"
                                ...
                                <div class="col-sm-9">
                                    ...
                                    <div class="input-group with-icon">
                                        <input type="text" class="form-control datepicker-max-today" name="compilation_date"
                                            value="{{ old('compilation_date', getFormattedDate($training->compilation_date)) }}"
                                            id="certificationComplitionDate" placeholder="{{ config('app.input_date_format') }}" required>
                                ...
                                <div class="col-sm-9">
                                    ...
                                    <div class="input-group with-icon">
                                        <input type="text" class="form-control datepicker" name="expire_date"
                                            value="{{ old('expire_date', getFormattedDate($training->expire_date)) }}"
                                            id="certificationExpireDate" placeholder="{{ config('app.input_date_format') }}" required >
                                        <i class="date-icon fa-solid fa-calendar-days"></i>
                                    </div>
                            ...
                            <div class="form-group row @error('score') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="score" min=".01" step="0.01"
                                        value="{{ old('score', $training->score) }}" id="certificationScore" placeholder="{{ __('Ex: 3.5') }}"
                                        required>
                                    @error('score')
                                    <p class="error-message">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="form-group row @error('cost') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="cost" step="0.01"
                                        value="{{ old('cost', $training->cost) }}" id="certificationCost" placeholder="{{ __('Ex: 1200.50') }}"
                                        required>
                                    @error('cost')
                                    <p class="error-message">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="form-group row @error('description') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <textarea type="text" name="description" class="form-control"
                                        ...
                                        rows="4">{{ old('description', $training->description) }}</textarea>
                                    @error('description')
                                    <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-sm-9">
                                    <div class="file-upload-section">
                                        ...
                                        <div class="display-input-image"
                                            style="display: {{ $training->documents ? '' : 'd-none' }}">
                                            @if($file_extension == 'pdf')
                                                <a href="{{ $training->getDocuments() }}" target="_blank"> View Pdf </a>
                                            @else
                                                <img src="{{ $training->getDocuments() }}"
                                                    alt="Preview Image" />
                                            @endif
                                            <button type="button"
