        ...
        <div class="p-sm-3">
            <div class="row">
                ...
                <div class="col-sm-6 form-group  @error('title') error @enderror">
                    ...
                    <input type="text" class="form-control" name="title" value="{{ old('title', $product->getTranslation('title')) }}"
                                    ...
                                    @foreach ($subSubCat->childrenCategories as $key => $subSub2Cat)
                                        @if ($subSub2Cat->categories)
                                            <option class="text-capitalize" value="{{ $subSub2Cat->id }}"
                                                {{ old('category_id') ?? $product->category_id == $subSub2Cat->id ? 'selected' : '' }}>
                ...
                <div class="col-sm-6 form-group  @error('weight') error @enderror">
                    ...
                    <input type="number" min="0" step="any" class="form-control" name="weight" value="{{ old('weight', $product->weight) }}"
                        placeholder="{{ __('Weight') }}" id="weight">
                    @error('weight')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @php
                    $dimensions = $product?->productDetails?->dimension;
                    $dimension = $dimensions ? json_decode($dimensions, true) : '';
                @endphp
                ...
                <div class="col-sm-6 form-group  @error('dimension') error @enderror">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <input type="number" min="0" step="any" class="form-control" name="length" value="{{ old('length', $dimension['length'] ?? '') }}" placeholder="{{ __('Length') }}" id="length">
                        </div>
                        <div class="col-md-4">
                            <input type="number" min="0" step="any" class="form-control" name="width" value="{{ old('width', $dimension['width'] ?? '') }}" placeholder="{{ __('Width') }}" id="width">
                        </div>
                        <div class="col-md-4">
                            <input type="number" min="0" step="any" class="form-control" name="height" value="{{ old('height', $dimension['height'] ?? '') }}" placeholder="{{ __('Height') }}" id="height">
                        </div>
                    </div>
                ...
                <div class="col-sm-6 form-group  @error('minimum_order_quantity') error @enderror">
                    ...
                    <input type="number" class="form-control" name="minimum_order_quantity" required
                        value="{{ old('minimum_order_quantity', $product->minimum_order_quantity) }}" placeholder="{{ __('Min. Order Quantity') }}">
                ...
                <div class="col-sm-6 form-group  @error('barcode') error @enderror">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" name="barcode" value="{{ old('barcode', $product->barcode) }}" id="barcode" placeholder="{{ __('Barcode') }}">
                ...
                @php
                    $stringKeywords = $product->getTranslation('tags');
                ...
                <div class="col-sm-12 form-group  @error('tags') error @enderror">
                    ...
                    <input type="text" name="tags[]" value="{{ old('tags[]', $key) }}" class="form-control tokenfield" id="productTags" placeholder="Write & Hit Enter" />
                    @error('tags')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
