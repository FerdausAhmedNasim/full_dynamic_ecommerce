    ...
    <div class="modal-dialog modal-default" role="document">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel"> <span> Change Payment Status </span> </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form method="post" action="{{ route('admin.user.seller.order.change.status', $seller_order->id) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-12">
                            <div class="p-sm-3">
                                ...
                                <div class="form-group row @error('status') error @enderror">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="select form-control" name="payment_status" id="status" style="width: 100%;" required>
                                            <option value="paid" {{ $seller_order->payment_status == 'paid' ? 'selected' : ''}}> Paid</option>
                                            <option value="unpaid" {{ $seller_order->payment_status == 'unpaid' ? 'selected' : ''}}> Unpaid</option>
                                        </select>
                                        @error('status')
