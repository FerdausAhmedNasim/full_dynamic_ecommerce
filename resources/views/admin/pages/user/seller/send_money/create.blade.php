...
@section('title', 'Send Money Create')
...
<div class="content-wrapper">
    ...
    <div class="content-header d-flex justify-content-start">
        ...
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Send Money Create' )) }}</h4>
        </div>
    </div>
    ...
    <div class="card shadow-sm col-lg-8  col-md-12 mt-3">
        <div class="card-body py-sm-4">
            <form method="post" class="form" action="{{ route('admin.user.seller.send.money.create', $user->id??'') }}"
                ...
                <div class="p-sm-3">
                    ...
                    <div class="form-group row @error('amount') error @enderror">
                        <label class="col-sm-3 col-form-label required"
                            for="amount">{{ __('Send Money Amount') }}</label>
                        <div class="col-sm-9">
                            <input type="number" class="form-control" name="amount" value="{{ old('amount') }}"
                            placeholder="{{ __('Send Money Amount') }}" required>
                            @error('amount')
                            <p class="error-message">{{ $message }}</p>
                            @enderror
                        </div>
