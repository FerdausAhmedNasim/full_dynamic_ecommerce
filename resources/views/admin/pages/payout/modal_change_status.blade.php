                                ...
                                <div class="form-group row @error('status') error @enderror">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="select form-control" name="status" id="status"
                                            ...
                                            @foreach(\App\Library\Enum::getPayoutStatusType() as $status)
                                                <option value="{{ strtolower($status) }}" {{ old('status', \App\Library\Enum::PAYOUT_STATUS_PENDING)  == $status ? "selected" : "" }}> {{ $status }}</option>
                                            @endforeach
                                        </select>
                                        @error('status')
...
<script type="text/javascript">
    ...
    function clickUpdateStatus(id) {
        $('#action').attr('action', '/payouts/' + id + '/status-change');
