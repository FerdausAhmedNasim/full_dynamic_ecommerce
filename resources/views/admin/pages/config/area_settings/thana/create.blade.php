...
@section('areaSettingsContent')
    ...
    <div class="content-header d-flex justify-content-start">
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('New Thana')) }}</h4>
        </div>
    </div>
    <hr>

    <form method="post" action="{{ route('admin.area.settings.thana.store') }}" enctype="multipart/form-data">
        @csrf
        <div class="p-sm-3">
            ...
            <div class="form-group row @error('en_name') error @enderror">
                ...
                <div class="col-sm-9">
                    <input type="text" class="form-control" name="en_name" value="{{ old('en_name') }}" placeholder="{{ __('Thana Name') }}" required>
                    @error('en_name')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
