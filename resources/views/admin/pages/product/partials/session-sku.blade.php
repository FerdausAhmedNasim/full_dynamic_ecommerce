        ...
        @for($i = 0 ; $i < session()->get('attributes'); $i++)
            <tr>
                <th scope="row" width="18%">
                    <label class="font-normal">{{ @old('variant_name')[$i] }}</label>
                    <input type="hidden" name="variant_name[{{$i}}]" value="{{ @old('variant_name')[$i] }}" class="form-control">
                    <input type="hidden" name="variant_ids[{{$i}}]" value="{{ @old('variant_ids')[$i] }}" class="form-control">
                </th>
                <td width="16%">
                    <input type="number" name="variant_price[{{$i}}]" value="{{ @old('variant_price')[$i] }}" min="0" step="any" class="form-control">
                </td>
                <td width="16%">
                    <input type="text" name="variant_sku[{{$i}}]" value="{{ @old('variant_sku')[$i] }}" class="form-control">
                ...
                <td width="16%">
                    <input type="number" name="variant_stock[{{$i}}]" value="{{ @old('variant_stock')[$i] }}" min="0" step="1" class="form-control">
                </td>
                <td width="29%">
                    <div>
                        <div class="form-group">
                            <div class="input-group gallery-modal"
                                ...
                                data-dismiss="modal">
                                <input type="hidden" name="variant_image[{{$i}}]" value="{{ @old('variant_image')[$i] }}" class="image-selected">
                                <span class="form-control">
                                    <span class="counter">0</span> {{ __('file') }}
                                </span>
                            ...
                            <div class="selected-media-box">
                                <div class="mt-2 gallery gallery-md d-flex">
                                    @if(@old('variant_image')[$i])
                                        <div class="selected-media mr-2 mb-2 mt-3 ml-0" data-id="{{ @old('variant_image')[$i] }}">
                                            @php
