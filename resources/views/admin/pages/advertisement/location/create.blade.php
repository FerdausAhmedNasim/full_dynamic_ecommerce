...
@section('title', __('New Ad Location'))
...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('admin.ad.location.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('New Ad Location')) }}</h4>
        </div>
    </div>

    <div class="card shadow-sm">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.ad.location.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        <div class="p-sm-3">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control select2" name="location" required>
                                        ...
                                        @foreach($locations as $key => $location)
                                            <option class="text-capitalize" value="{{ $key }}" {{ (old("location") == $key) ? "selected" : "" }}>
                                                {{ $location }}</option>
                                        @endforeach
                                    </select>
                                    @error('location')
                            ...
                            {{-- <div class="form-group row @error('amount') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="amount"
                                        value="{{ old('amount') }}" placeholder="{{ __('Amount') }}" required>
                                    @error('amount')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
