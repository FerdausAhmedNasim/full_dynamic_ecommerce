...
<body>
    <div class="invoice-box" style="page-break-inside: avoid;">
        <div>
                    ...
                    <table>
                        <tr class="border-top">
                            <td style="width: 60%">
                                <div>
                                    <b> Billed To:</b> <br />
                                    <span class="">{{ $order?->order->customer ? $order?->order->customer?->full_name : $order?->order->order_person_name }}</span> <br />
                                    Address: {{ $order?->order->customer ? $order?->order->customer?->address : $order?->order->order_person_address }}<br />
                                    Phone: {{ $order?->order->customer ? $order?->order->customer?->phone : $order?->order->order_person_phone }}<br />
                                </div>
                            </td>
                            ...
                            <td style="width: 40%">
                                <div class="text-grey-m2">
                                    ...
                                    <span class="text-600">{{ getCompanyAddress() ?? 'N/A' }}</span><br />
                                    <span class="text-600">{{ settings('phone') == '' ? 'N/A' : settings('phone') }}</span><br />
                                    <span class="text-600">{{ settings('email') == '' ? 'N/A' : settings('email') }}</span><br />
                                    {{-- <span class="text-600">
                                        <a class="text-primary-m1" target="_blank"
                                            href="{{ settings('website') && settings('website') != '' ? settings('website') : '#' }}">
                                            {{ settings('website') && settings('website') != '' ? settings('website') : 'N/A' }}
                                        </a>
                                    </span> --}}
                                </div>
                            </td>
        ...
        <div
            style="padding-bottom: 10px; padding: 8px 0;background-color: #f1f1f1;margin-bottom: 18px;     display: flex;
    justify-content: center;
    align-items: center; font-size: 12px;">
            <span class="mx-2">Invoice Number: {{ $order->order->invoice_id }}</span>
            ...
            <span>{{ \App\Library\Enum::getPaymentStatusType($order?->payment_status) }}</span>
            <br />
        </div>
        ...
        <div class="border-top">
            ...
            <table class="table table-striped">
                <tr style="background-color: #07a769; color: white;">
                    ...
                    @foreach ($order->sellerorderDetails as $key => $orderDetail)
                        ...
                        <tr>
                            @php
                                $image = base64_encode(
                                    file_get_contents(
                                        $orderDetail->load('product')?->product?->Thumbnail
                                            ? public_path($orderDetail?->product->thumbnail)
                                            : resource_path('images/noimage.jpg'),
                                    ),
                                );
                            @endphp
                            <td>
                                <img style="margin: 0 auto;" src="data:image/png;base64, {{ $image }}"
                                    width="50" />
                            </td>
