    ...
    <div class="modal-dialog modal-default" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ route('admin.order.change_status', $order->id) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-12">
                            <div class="p-sm-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="select form-control" name="status" id="status" style="width: 100%;" required>
                                            @foreach($order_status as $key => $status)
                                                @if (in_array($key, ['returned', 'incomplete', 'partial_returned']))
                                                    @continue
                                                @endif
...
<script type="text/javascript">
    ...
    const initialOrderStatus = "{{ $order->order->order_status }}";
        ...
        function updateStatusOptions(status) {
            ...
            switch (status) {
                ...
                case 'return':
                    ...
                    break;
                case 'processing':
                    disableOptions(['pending', 'canceled', 'processing', 'delivered', 'returned', 'incomplete']);
                    break;
                case 'shipped':
                    disableOptions(['pending', 'canceled', 'processing', 'shipped', 'returned', 'incomplete']);
                    break;
                default:
