...
@section('title', __('Update Withdraw'))
...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('admin.withdraw.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Withdraw')) }}</h4>
        </div>
    ...
    <div class="card shadow-sm col-md-8">
        <div class="card-body py-sm-4">
            <form method="post" class="form" action="{{ route('admin.withdraw.update', $withdraw->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="p-sm-3">
                            <div class="form-group row @error('amount') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="amount" id="amount" value="{{ old('amount',$withdraw->amount) }}" step="any"
                            ...
                            <div class="form-group row @error('note') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <textarea type="text" rows="10" class="form-control" name="note"
                                        placeholder="{{ __('Note') }}" required>{{ old('note',$withdraw->note) }}</textarea>
                                    @error('note')
                                    <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
