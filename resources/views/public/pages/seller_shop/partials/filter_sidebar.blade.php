...
$categories = \App\Models\SellerCategory::where('seller_id', $seller->id)
                ->with('category.languages', 'category.childrenCategories.childrenCategories.childrenCategories')
                ->get();
$brand_ids = \App\Models\Product::where('seller_id', $seller->id)->where('brand_id','!=',null)->groupBy('brand_id')->pluck('brand_id');
   
$brands = \App\Models\Brand::active()->whereIn('id', $brand_ids)->get();
                            ...
                            <ul class="category-list custom-padding custom-height">
                                ...
                                <li>
                                    <div class="form-check ps-0 m-0 category-list-box">
                                        ...
                                        @if(isset($category)) @if($category->id == $cat->category->id)
                                        checked @endif @endif name="category[]" onclick="filter()"
                                            value="{{$cat->category->id}}" id="category{{$cat->category->id}}">
                                        <label class="form-check-label" for="category{{$cat->category->id}}">
                                            <span class="name">{{ $cat->category->getTranslation('title') }}</span>
